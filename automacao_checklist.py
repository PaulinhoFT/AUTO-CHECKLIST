...
def carregar_pre_registros():
    ...
    if not pre_registros:
        pre_registros = {
            ...
            "Botão 15 - abertura de OS setor tec IA": "Por favor, analise este atendimento e extraia as informações necessárias para a abertura de uma Ordem de Serviço (OS) técnica. Preencha os seguintes campos de forma clara e objetiva:\n\nRECLAMAÇÃO DO ASSINANTE: [Detalhe o problema relatado]\nPROCEDIMENTOS REALIZADOS: [Liste SOMENTE os testes e ações técnicas feitas remotamente. Não inclua confirmação de endereço ou informações não técnicas]\nORIENTAÇÃO AO TEC. DE CAMPO: [O que o técnico precisará verificar ou fazer no local]\n\nIMPORTANTE: Todo o texto gerado deve estar EXCLUSIVAMENTE em letras MAIÚSCULAS para compatibilidade com nosso sistema."
        }
